...
strMCMC_BoB_mixed <- function(Data_mixed,iterations,thin_factor,q_resample, INDICATOR_VEC){


  n <- nrow(Data_mixed)    # number of nodes
  ...
  m <- ncol(Data_mixed)    # number of observations
  ...
  indicis_1 = which(INDICATOR_VEC==1)

  v_max = max(Data_mixed[indicis_1,])
  ...
  for (node in 1:n){
	...
	if (INDICATOR_VEC[node]==1){

    		v_max = max(Data_mixed[node,])
		VEC[node] = v_max
    		for (v in 1:v_max){IND[[node,v]] = which(Data_mixed[node,]==v)}
		}
